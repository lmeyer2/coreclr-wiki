...
Upon completing a test run, you may find one or more tests have failed. If you wish to re-run a failed test, please follow the following steps:
...
4. Add the .NET CoreFX contract references, as required, via the Nuget Package Manager in Visual Studio. *Make sure this does not change the csproj. If it does, then undo the change in the csproj.*
5. Add any other projects as a dependency, if needed.
6. Build the test.
7. Follow the steps to re-run a failed test to validate the new test.
